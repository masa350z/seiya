...
class Sanren120(boatdata.BoatDataset):
    ...
    def set_dataset(self, batch_size, odds_mask_k=0):
        seikai_odds = -1*np.sum(self.label-1, axis=1)
        odds_mask = seikai_odds > odds_mask_k
        ...
        self.x_train, self.x_valid, self.x_test = boatdata.split_data(inp[odds_mask])
        self.field_train, self.field_valid, self.field_test = boatdata.split_data(inp_fe[odds_mask])
        self.pre_train, self.pre_valid, self.pre_test = boatdata.split_data(pre_info[odds_mask])
        self.odds_train, self.odds_valid, self.odds_test = boatdata.split_data(self.odds[odds_mask])
        self.y_train, self.y_valid, self.y_test = boatdata.split_data(self.label[odds_mask])
        ...
        for epoch in range(epochs):
            ...
            # valid lossが減少しなかった場合、保存しておいた最良の重みをロード
                ...
                if freeze == 0:
                    ...
                    self.model_weights_random_init(init_ratio=0.001)
                    freeze = k_freeze
...
class OddsAttention(tf.keras.Model):
    def __init__(self):
        super(OddsAttention, self).__init__()

        self.dense01 = layers.Dense(1024, activation='relu')
        self.dense02 = layers.Dense(1024, activation='relu')

        self.attention = layers.Dense(120, activation='sigmoid')

    def call(self, inputs, outputs):
        x = self.dense01(inputs)
        x = self.dense02(x)

        return outputs*self.attention(x)
...
class BoatTransformer(tf.keras.Model):
    ...
    def __init__(self):
        ...
        self.odds_attention = OddsAttention()
    ...
    def call(self, inputs):
        ...
        conc = layers.concatenate([field, odds, x01, x02, x03, x04, x05, x06])
        x = self.layer01(conc)

        x = self.layer02(x)
        x = self.output_layer(x)

        return self.odds_attention(conc, x)
...
# %%
bt = Sanren120(0.2)
bt.set_dataset(batch_size=120, odds_mask_k=0)
...
# %%
bt.start_training(epochs=100, weight_name='datas/sanren120/boat_transformer', k_freeze=3)
# %%
bt.model.load_weights('datas/sanren120/boat_transformer')
# %%
pred = bt.model.predict(bt.train)
pred_ = pred*bt.y_train
# %%
print(np.sum(pred_)/len(pred_))
# %%
seikai_odds = -1*np.sum(bt.label-1, axis=1)
# %%
