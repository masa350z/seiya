...
class BoatNN(boatdata.BoatDataset):
    ...
    def set_dataset(self, batch_size, mode='sanren'):
        data_x = tf.data.Dataset.from_tensor_slices((bt.racers, bt.ar_grade_num,
                                                     bt.ar_ze, bt.ar_to,
                                                     bt.conditions, bt.ar_incourse_num,
                                                     bt.ar_field-1, bt.weather-1, bt.wind_vector,
                                                     bt.tenji_time, bt.tenji_start_time,
                                                     bt.sanren_odds))
        if mode == 'sanren':
            data_y = tf.data.Dataset.from_tensor_slices(bt.ret_sanren_onehot())
        else:
            data_y = tf.data.Dataset.from_tensor_slices(bt.ret_niren_onehot())
...
class SEIYA(tf.keras.Model):
    ...
    def call(self, input):
        ...
        racers = layers.Flatten()(self.racer_encoder01(racer_vectors))
        racers = self.racer_encoder02(racers)
        ...
        odds = self.odds_encoder(sanren_odds)
...
# %%
bt = BoatNN(unknown_rate=0, k_std=20000000)
...
weight_name = 'datas/pred_sanren/best_weights'
# %%
race_field = 1
bt.ar_field == race_field
# %%
np.tile(True, len(bt.ar_field))
# %%
...
# %%
res = model.predict(bt.test_dataset)
# %%
res[:10]/odds_sanren[:10]
# %%
bt.sanren_odds
# %%
model = SEIYA(feature_dim)
